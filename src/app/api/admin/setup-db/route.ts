...
export async function POST(request: NextRequest) {
  try {
    console.log('🔧 Starting Prisma Accelerate database setup...')
    ...
    try {
      ...
      const userCount = await setupPrisma.user.count()
      console.log('✅ Tables exist, users:', userCount)
    } catch (error) {
      console.log('❌ Tables do not exist:', error)
      return NextResponse.json({
        error: 'Database tables do not exist',
        message: 'Prisma Accelerate requires tables to be created via direct database connection',
        instructions: [
          '1. Get your direct PostgreSQL URL (not the Prisma Accelerate URL)',
          '2. Set DIRECT_DATABASE_URL in Vercel environment variables',
          '3. Run: npx prisma db push with the direct URL',
          '4. Then call this endpoint again'
        ],
        accelerateUrl: 'You are using Prisma Accelerate - tables must be created via direct connection'
      }, { status: 500 })
    }
    
    // Check if questions exist
    const questionCount = await setupPrisma.question.count()
    if (questionCount > 0) {
      const stats = {
        users: await setupPrisma.user.count(),
        questions: questionCount,
        testSessions: await setupPrisma.testSession.count()
      }
    ...
    const questions = [
      ...
      },
      {
        ...
        difficulty: 3,
        estimatedTime: 120,
        passage: 'The old lighthouse stood sentinel on the rocky coast, its weathered walls bearing witness to countless storms. Sarah approached it with a mixture of excitement and trepidation. She had inherited this structure from her great-aunt, along with the responsibility of deciding its fate. The local historical society wanted to preserve it, while developers saw prime real estate. As she climbed the spiral staircase, each step echoed with the weight of history and the burden of choice. The view from the top was breathtaking—endless ocean stretching to the horizon, waves crashing against the rocks below.',
      ...
      },
      {
        ...
        difficulty: 4,
        estimatedTime: 150,
        passage: 'Recent studies in marine biology have revealed fascinating insights into the communication patterns of dolphins. Using sophisticated underwater recording equipment, researchers have discovered that dolphins employ a complex system of clicks, whistles, and body language to convey information. Each dolphin appears to have a unique "signature whistle" that functions much like a name, allowing individuals to identify themselves to others in their pod. Furthermore, dolphins have been observed modifying their communication patterns based on environmental factors such as water clarity and ambient noise levels.',
        ...
        solution: 'The passage explicitly states that dolphins "modify their communication patterns based on environmental factors" and calls this adaptive behavior evidence of cognitive flexibility.',
    ...
    for (const question of questions) {
      await setupPrisma.question.create({ data: question })
    }
    ...
    const finalStats = {
      users: await setupPrisma.user.count(),
      questions: await setupPrisma.question.count(),
      testSessions: await setupPrisma.testSession.count()
    }

    return NextResponse.json({
      message: 'Database initialized successfully with Prisma Accelerate',
